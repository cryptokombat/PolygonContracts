...
import { TestERC20, RewardStaking } from '../typechain-types'
...
const setupTest = deployments.createFixture(async ({ deployments, getNamedAccounts, ethers }, options) => {
  ...
  const stakeToken = (await ethers.getContract('TestERC20', deployer)) as TestERC20
  const rewardToken = (await ethers.getContract('TestERC20', deployer)) as TestERC20
  const reward = (await ethers.getContract('RewardStaking', deployer)) as RewardStaking
})

context('LQTYStaking revenue share tests', () => {
