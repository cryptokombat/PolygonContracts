...
export const mint = async (
  ...
  const signer = new Wallet(process.env.BRIDGE_TEST_PK!, departureChain.getProvider())

  for await (const uri of uris) {
    const nftResult = await factory.mint(departureChain, signer, {
      contract,
      uris: [uri],
    } as NftMintArgs)
...
export const nftList = async (departureChain: Web3Helper, chainName: string, factory: ChainFactory) => {
  ...
  const NFTs = await factory.nftList<EthNftInfo>(departureChain, signer.address)
